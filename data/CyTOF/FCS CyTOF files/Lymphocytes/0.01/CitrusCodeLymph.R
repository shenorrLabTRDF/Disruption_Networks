...
sample<-read.FCS("/storage/md_shenorr/shirang/Remicade.CyTOF.Lymph.Fcs.Files/HR-22-V1_cells_found_normalized_Lymphocytes.fcs")
...
if(DEFINE_PARAMETERS) {
  ...
  dataDirectory = "/storage/md_shenorr/shirang/Remicade.CyTOF.Lymph.Fcs.Files"
  #args <- commandArgs(trailingOnly = TRUE)
	citrusRes$minClustSzPrcnt = 0.01
  ...
  fileList = data.frame(list.files(dataDirectory,pattern='.fcs'), stringsAsFactors=FALSE)
  pheno=read.csv("/storage/md_shenorr/shirang/Remicade.CyTOF.Lymph.Fcs.Files/fcs.annotation.csv",header=TRUE)
citrusRes$sample.labels = as.factor(pheno[match(fileList[,1],pheno$name),'pheno'])

markersAno=read.csv(file = "/storage/md_shenorr/shirang/Remicade.CyTOF.Lymph.Fcs.Files/clusteringfornon-grans citrus_ES.csv",header = TRUE, stringsAsFactors=FALSE)
